...
data_preprocess <- function(filename, threshold = 2.5){
  ...
  if (st == "4"){ #In study 4 the time-column is called "TimeMs" which is different from all other studies. Therefore we tell the function to name the column "time" for study 4
    df <- df %>% rename(
      time = TimeMs
    )
  }
...
#First we look at study 1 HR1
d1 <- filter(phys_data, Study == 1)
d1 <- group_by(d1, Group, Trial) %>% 
  mutate(time1 = seq(n())) #mutate creates a new column, time1, and seq() starts over each time

ggplot(data = d1) + geom_line(aes(time1, HR1, color= "red")) + 
...
d2 <- filter(phys_data, Study == 2)
d2 <- group_by(d2, Group, Trial) %>% 
  mutate(time1 = seq(n()))

ggplot(data = d2) + geom_line(aes(time1, HR1, color= "red")) + 
  geom_line(aes(time1, HR2, color= "blue")) + 
  facet_wrap(Group~Trial)
...
d3 <- filter(phys_data, Study == 3)
d3 <- group_by(d3, Group, Trial) %>% 
  mutate(time1 = seq(n()))

ggplot(data = d3) + geom_line(aes(time1, HR1, color= "red")) + 
  xlim(0,200) +
  geom_line(aes(time1, HR2, color= "blue")) + 
  facet_wrap(Group~Trial)
...
d4 <- filter(phys_data, Study == 4)
d4 <- group_by(d4, Group, Trial) %>% 
  mutate(time1 = seq(n()))

ggplot(data = d4) + geom_line(aes(time1, HR1, color= "red")) + 
  #xlim(0,400) +
  geom_line(aes(time1, HR2, color= "blue")) + 
  facet_wrap(Group~Trial)
...
#First we look at study 1 Resperation 
d1 <- filter(phys_data, Study == 1)
d1 <- group_by(d1, Group, Trial) %>% 
  mutate(time1 = seq(n())) #mutate creates a new column, time1, and seq() starts over each time

ggplot(data = d1) + geom_line(aes(time1, Resp1, color= "red")) + 
...
ggplot(data = d2) + geom_line(aes(time1, Resp1, color= "red")) + 
  geom_line(aes(time1, Resp2, color= "blue")) + 
  facet_wrap(Group~Trial)
...
ggplot(data = d3) + geom_line(aes(time1, Resp1, color= "red")) + 
  xlim(0,200) +
  geom_line(aes(time1, Resp2, color= "blue")) + 
  facet_wrap(Group~Trial)
...
ggplot(data = d4) + geom_line(aes(time1, Resp1, color= "red")) + 
  #xlim(0,400) +
  geom_line(aes(time1, Resp2, color= "blue")) + 
  facet_wrap(Group~Trial)
...
ggplot(data = d4_clean) + geom_line(aes(time1, Resp1, color= "red")) + 
  ...
  geom_line(aes(time1, Resp2, color= "blue")) + 
  facet_wrap(Group~Trial)
#When we look at the plots we see flat signals (bad data). We also see that in some places our threshold of 2.5 standard deviations has removed maybe too much data (it has cut a big chunk off the data). We could replace the flat signals with NAs. We can also see that the first two trials had a loose cable, which means that the signal did not show up that much - thus, there is a lot of noise and after it has become scaled it oscillates much more than it would normally do. We need to cut this away. We should be careful to remove good data - the solution is to replace bad data with NA and then remove the NAs. When we report, we should remember to count the number of participants - the number that the count function gives us is not the actual number, because we have removed the data from the participants with NAs.

#When we look at the respiration data we should look for data that suggests that the respiration belt has been very tight and therefore it has not been capturing variation. We should remove this data.

#We replace the bad signal (flat signals) with NAs for a particular study, group, trial, condition etc.?

# filtering out the first bad data for study 2, group 6 and trail 3, in order to remove the bad data. 
...
# filtering out the bad data for study 2, group 9 and trail 1, in order to remove the bad data. 
...
# filtering out the first bad file, in order to remove the bad data. 
...
#What if it is only a small part of the signal that we want to replace with NAs?
#write a function that checks if the interval around a certain data point is below 2.5 standard deviation - adjust the outlier function!!
...
HR_change_other <- gather(phys_data_2019, # data
...
# Merging 2 of the above created datasets into one, by time, participant, study, group, condition, trial
change <- merge(HR_change, Resp_change, by=c("time","participant", "Study", "Group", "Condition", "Trial"),all=T)

# Merging the other 2 of the above created datasets into one, by time, participant, study, group, condition, trial
self <- merge(Resp_self, HR_self,by=c("time","participant", "Study", "Group", "Condition", "Trial"), all=T)

other <- merge(Resp_other, HR_other,by=c("time","participant", "Study", "Group", "Condition", "Trial"), all=T)

change_other <- merge(HR_change_other, Resp_change_other,by=c("time","participant", "Study", "Group", "Condition", "Trial"), all=T)

lead <- merge(Resp_lead, HR_lead,by=c("time","participant", "Study", "Group", "Condition", "Trial"), all=T)


change_self <- merge(change, self ,by=c("time","participant", "Study", "Group", "Condition", "Trial"), all=T)

other_change_other <- merge(change_other, other,by=c("time","participant", "Study", "Group", "Condition", "Trial"), all=T)

lead_change_self <- merge(lead, change_self,by=c("time","participant", "Study", "Group", "Condition", "Trial"), all=T)

# Merging the 2 merged dataset into one dataset, by time, participant, study, group, consition, trial
phys_data_long <- merge(lead_change_self, other_change_other, by=c("time","participant", "Study", "Group", "Condition", "Trial"), all = T)
...
phys_test <- filter(phys_data_long, participant == 31, participant == 11, participant == 21)
# Model change as a function of own and other previous state 
model <- lmer(HR_change_self ~  0 + (HR_self + HR_other) : Condition + 
                (0 + (HR_self + HR_other) : Condition | participant) + 
                (0 + (HR_self + HR_other) : Condition | Group), data = phys_data_long)
